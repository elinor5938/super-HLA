...
def main():
    ...
    # ── Stage 0 ──────────────────────────────────────────────────────────────
    print("\n" + "-" * 60)
    print("[Sub-stage 0] Loading MCMC simulation data...")
    print("-" * 60)
    _sys.stdout.flush()
    t0 = time.time()
    stage0_data = run_stage0()
    print(f"[Sub-stage 0] Done in {time.time() - t0:.1f}s — "
          f"{len(stage0_data['threshold_8_hla_passing_peptides'])} HLA combinations with >=8 binders")
    _sys.stdout.flush()

    # ── Stage 1 ──────────────────────────────────────────────────────────────
    print("\n" + "-" * 60)
    print("[Sub-stage 1] Two-round CD-HIT clustering")
    print("-" * 60)
    _sys.stdout.flush()
    t1 = time.time()
    stage1_data = run_stage1(stage0_data)
    print(f"[Sub-stage 1] Done in {time.time() - t1:.1f}s — "
          f"{len(stage1_data['consensus_peptides'])} representative peptides")
    _sys.stdout.flush()

    # ── Stage 2 ──────────────────────────────────────────────────────────────
    print("\n" + "-" * 60)
    print("[Sub-stage 2] Synthesis difficulty filter")
    print("-" * 60)
    _sys.stdout.flush()
    t2 = time.time()
    stage2_data = run_stage2(stage1_data["consensus_peptides"])
    print(f"[Sub-stage 2] Done in {time.time() - t2:.1f}s — "
    ...
    if _candidate_path and os.path.exists(_candidate_path):
        ...
        print(f"\n[Sub-stage 3] Removed previous candidate file: {_candidate_path}")
        _sys.stdout.flush()

    if not stage2_data["filtered_peptides"]:
        print("\n[Sub-stage 3] No peptides to validate — skipping.")
        stage3_data = {"scores_df": None, "top_primary": [], "top_net40": [], "top_flurry": []}
    else:
        print("\n" + "-" * 60)
        print("[Sub-stage 3] MHC binding cross-validation")
        print("-" * 60)
        _sys.stdout.flush()
        t3 = time.time()
        stage3_data = run_stage3(stage2_data["filtered_peptides"])
        print(f"[Sub-stage 3] Done in {time.time() - t3:.1f}s")
        _sys.stdout.flush()
